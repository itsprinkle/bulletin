...
class MongoHandler(logging.Handler):
    ...
    def _connect(self):
        ...
        try:
            self.connection = MongoClient(host=self.host, port=self.port, **self.options)
        except PyMongoError:
        ...
        if self.capped:
            try:  # We don't want to override the capped collection (and it throws an error anyway)
                ...
                self.collection = MongoClient(self.db, self.collection_name, capped=True, max=self.capped_max, size=self.capped_size)
